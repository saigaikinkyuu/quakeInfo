...
function drawMap(){
  ...
  fetch('https://www.jma.go.jp/bosai/tsunami/data/list.json')
    ...
    .then(datas => {
      if(datas[0]){
        fetch('https://www.jma.go.jp/bosai/tsunami/data/' + datas[0].json)
        .then(response => response.json())
        .then(data => {
          var pointFlag = true
          ...
          let forecast_items = data.Body.Tsunami.Forecast.Item
		...
		return {
		 ...
		 onEachFeature: function (feature, layer, forecast_items) {
